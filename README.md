...
2. **Import modified localization files to merge using the `Add`/`Remove` buttons.** The files are merged upwards in the order that they appear in the list, meaning files lower in the list take priority if there are conflicts. Organize imported files accordingly using the `Move up` and `Move down` buttons.
